...
survival_analysis_continuous = function(CAT, CLINICS, clinvar = 'DAYS_TO_LAST_FOLLOWUP' ){
  library(reshape)
  ...
  if( n.dead.cases > 3 ) {
    for (predictor in colnames(CAT)){
      ...
      if( length(unique(gender)) == 1 )   
        mysurfit2 = coxph(Surv(stime, status) ~ age+X)   #  summary(mysurfit2)
      X_pval = summary(mysurfit2)$coefficients['X', 'Pr(>|z|)']  # log-rank test
      X_coef = unname(coef(mysurfit2)['X'])
      RESULTS[[predictor]] = c(predictor= predictor, X_pval = X_pval, 'X_coef' = X_coef ) 
      # mysurvdiff = survdiff(Surv(stime, status) ~ X) # log-rank test
      X_pval[ is.na(X_pval) ] = 10
  ...
  mRESULTS <- mRESULTS[ ,c(1,2,5)]
#   fit <- coxph(Surv(stime, status) ~ age+gender+X)
#   plot(survfit(fit)) 
  return(mRESULTS)
}

survival_KM_plot <- function(data,title) {
  library(survminer)
