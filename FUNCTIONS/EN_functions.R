...
run_EN_setting4_MacauFactorization <- function(RES, features_col,features_row, fold, iteration, alpha) {
  ...
  for (i in 1:iteration) {     ##  start iteration of k fold cross validation
    ...
    for (i in 1:1) {
      ...
      # Transform into Macau model
      train_feat  <- cbind(train_feat, create_Macau_combo(train_feat) )
      xTrain_feat <- cbind(xTrain_feat, create_Macau_combo(xTrain_feat) ) 
      ...
      # Transform into Macau model
      test_feat  <- cbind(test_feat, create_Macau_combo(test_feat) ) 
...
run_EN_logit <- function(RES,index,features, fold, iteration, alpha) {
  ...
  result <- mean(result_table, na.rm=T)
  return(result)
}
