...
scatter_plot_tissue <- function(tissue_name,features,pathway_name,target_name,drug_number=1,text_size=26,title_size=2.5) {
  ...
  g <- ggplot(df, aes(a, b, color = b)) + 
    ...
    labs(x = pathway_name, y="IC50" ) + ggtitle(paste0(drug," (",target_name,")"," in ",tissue_name)) + 
...
library(cowplot)
result_folder <- "/Users/miyang/Documents/RWTH_Aachen/MACAU_PROJECT/PLOTS/GDSC_target_progeny_Scatter_Plot/"
...
figure1 <- scatter_plot_tissue( "liver", "progeny11", "TGFb", "Farnesyl-transferase (FNTA)" )  
...
par(mfrow=c(2,1))
figure1 <- scatter_plot_tissue( "lung_SCLC", "progeny11", "VEGF", "TOP1", 2 )
figure2 <- scatter_plot_tissue( "lung_SCLC", "progeny11", "MAPK", "MCL1"  )  
plot_grid(figure2, figure1, ncol = 1, nrow = 2 )
dev.off()
...
figure2 <- scatter_plot_tissue( "soft_tissue", "progeny11","TNFa","dsDNA break" , 2)
plot_grid(figure2, figure1, ncol = 1, nrow = 2 )
dev.off()
...
par(mfrow=c(1,2))
figure1 <- scatter_plot_tissue( "ovary", "progeny11", "p53", "MDM2",1)
figure2 <- scatter_plot_tissue( "lymphoma" , "progeny11", "NFkB", "Antimetabolite",1)
plot_grid(figure1, figure2, ncol = 2, nrow = 1 )
dev.off()
...
```{r include=FALSE, cache=FALSE}
library(cowplot)
result_folder <- "/Users/miyang/Documents/RWTH_Aachen/MACAU_PROJECT/PLOTS/GDSC_target_progeny_Scatter_Plot/00_antagonist_tissue/"

pdf(file=paste0(result_folder,"VEGF_Microtubule_stabiliser",".pdf"), width = 22, height = 11,  compress=TRUE, onefile = F)
par(mfrow=c(1,2))
...
plot_grid(figure1, figure2, ncol = 1, nrow = 2 )
dev.off()

pdf(file=paste0(result_folder,"Hypoxia_ALK",".pdf"), width = 22, height = 11,  compress=TRUE, onefile = F)
par(mfrow=c(1,2))
...
pdf(file=paste0(result_folder,"TNFa_HDAC1",".pdf"), width = 22, height = 11,  compress=TRUE, onefile = F)
par(mfrow=c(1,2))
...
pdf(file=paste0(result_folder,"TNFa_PI3Kbeta",".pdf"), width = 22, height = 11,  compress=TRUE, onefile = F)
par(mfrow=c(1,2))
...
pdf(file=paste0(result_folder,"NFkB_ERBB2",".pdf"), width = 22, height = 11,  compress=TRUE, onefile = F)
par(mfrow=c(1,2))
...
pdf(file=paste0(result_folder,"EGFR_MEK2",".pdf"), width = 22, height = 11,  compress=TRUE, onefile = F)
par(mfrow=c(1,2))
...
scatter_plot_tissue_2_features <- function(tissue_name, features_1, features_1_name,features_2,features_2_name, target_name, drug_number=1, text_size=26, title_size=2.5) {
  features_1 <- read.csv(paste0("/Users/miyang/Documents/RWTH_Aachen/SANGER_DATA/TISSUE/",tissue_name,"/",features_1),row.names = 1)
  ...
  features_2 <- read.csv(paste0("/Users/miyang/Documents/RWTH_Aachen/SANGER_DATA/TISSUE/",tissue_name,"/",features_2),row.names = 1)
